...
function processModuleFilter(module) {
  ...
  if (module['path'].indexOf('__prelude__') >= 0) {
    console.log('过滤掉__prelude__' + module['path'])
    return false;
  }
  ...
  if (module['path'].indexOf(pathSep + 'node_modules' + pathSep) > 0) {
    ...
    */
    if ('js' + pathSep + 'script' + pathSep + 'virtual' == module['output'][0]['type']) {
      console.log('node_modules加入业务包的' + module.path)
      return true;
    }
    return false;
  }

  // if (module['path'].indexOf(pathSep + 'Component' + pathSep) > 0) {
  //   console.log('过滤掉组件' + module['path'])
  //   return false;
  // }
  console.log('加入业务包的' + module.path)
  //其他就是应用代码
  return true;
}
